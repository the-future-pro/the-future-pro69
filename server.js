...
// preview blur, credits charging, personas CRUD basic, + Premium UI & debug links.
...
// Mock subscribe (test) — set tier & extend sub
...
// ===================== Premium UI + Debug helpers =====================
app.get('/premium', (req, res) => {
  res.type('html').send(`<!doctype html>
<html lang="ro"><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1">
<title>Premium</title>
<style>
  body{background:#0b0f14;color:#e6e6e6;font-family:system-ui,Segoe UI,Roboto,sans-serif;margin:0;padding:24px}
  h1{margin:0 0 16px}
  .row{margin:12px 0}
  input,button{font-size:16px;padding:10px 12px;border-radius:10px;border:1px solid #2a3648;background:#121821;color:#fff}
  button{background:#3c6df6;border-color:#3c6df6;margin-right:8px}
  button.gray{background:#1b2433;border-color:#1b2433}
  pre{background:#0f1420;border:1px solid #1f2a3a;padding:12px;border-radius:10px;overflow:auto;min-height:120px}
  a{color:#8ab4ff}
</style>
<h1>Premium</h1>

<div class="row">
  <input id="email" placeholder="email" value="test@example.com" style="min-width:260px">
  <button id="btnLogin">Login</button>
</div>

<div class="row">
  <button id="tBasic"  class="gray">BASIC</button>
  <button id="tPlus"   class="gray">PLUS</button>
  <button id="tPro">PRO</button>
</div>

<div class="row">
  <button id="btnMe" class="gray">Vezi /api/me</button>
  <a href="/api/debug/cookies" target="_blank">Vezi cookie</a>
  <a href="/api/debug/logout"  style="margin-left:12px">Logout</a>
</div>

<pre id="out">{}</pre>

<script>
  const out=document.querySelector('#out'), email=document.querySelector('#email');
  ...
  document.querySelector('#btnLogin').onclick = ()=> post('/api/login',{ email: email.value.trim() });
  document.querySelector('#tBasic').onclick   = ()=> post('/api/sub/mock-activate',{ tier:'BASIC' });
  document.querySelector('#tPlus').onclick    = ()=> post('/api/sub/mock-activate',{ tier:'PLUS'  });
  document.querySelector('#tPro').onclick     = ()=> post('/api/sub/mock-activate',{ tier:'PRO'   });
  document.querySelector('#btnMe').onclick    = async()=>{
    const r = await fetch('/api/me',{credentials:'include'}); log(await r.json());
  };
</script>
</html>`);
});
...
app.get('/api/debug/login', async (req,res)=>{
  try{
    const email = String(req.query.email||'debug@example.com').toLowerCase();
    let u = await get(`SELECT * FROM users WHERE email=?`,[email]);
    if(!u){
      const r = await run(`INSERT INTO users(email) VALUES(?)`,[email]);
      u = await get(`SELECT * FROM users WHERE id=?`,[r.lastID]);
...
app.get('/api/debug/mock-activate/:tier', async (req,res)=>{
  if(!req.session?.user_id) return res.status(401).json({ok:false,error:'login_required'});
  const tier = String(req.params.tier||'BASIC').toUpperCase();
  ...
  res.json({ ok:true, subActive:true, subTier:tier, until });
});
// =====================================================================
