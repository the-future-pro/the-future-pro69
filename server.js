// server.js (ESM, prod-ready) — Replicate + lipire segmente cu ffmpeg + endpoint deschis pt. test
...
// descarcă URL într-un fișier (Node 20 are fetch nativ)
...
// --- varianta cu sesiune + abonament (UI normal)
app.post("/api/video", requireLogin, subRequired, handlerGenerateVideo);

// --- ENDPOINT DESCHIS pentru test din telefon (ReqBin / Insomnia / Postman)
// Trimite header: X-Admin-Token: <ADMIN_TOKEN din env>
...
async function handlerGenerateVideo(req, res) {
  try {
    ...
    // DEMO fallback dacă nu avem cheie/model
